...
const TEAL  = '0D7480';  // ティール（ニッチ戦略アクセント）
...
// ══════════════════════════════════════════════════════════
{
  ...
  const qas = [
    {
      q: '本当に生協と契約できるの？',
      a: [
        '競合大手（アリスシステム等）は大規模校中心。留学生比率が高い中小規模の生協は未提携校が多数残っており、先手で入れば独占できる。',
        '生協が断っても専門学校・日本語学校・KB経由など複数チャンネルあり。1チャンネル頼みではない構造。',
        'まず1〜2校とPoC提携 → 実績をつくって横展開する。最初から50校狙わない。',
      ],
    },
    {
      q: '留学生は本当に借りるの？',
      a: [
        '来日初日に現金5万円を一括で払える留学生は多くない。月2,083円に分割すれば収入が不安定な学生でも契約しやすい。',
        '冷蔵庫・洗濯機・電子レンジは生活必需品。「借りない・買わない」という選択肢が事実上ない。',
        '類似サービス（かして！どっとこむ等）がすでに市場で成立しており、需要の実在は証明済み。',
      ],
    },
    {
      q: '誰が動く？人手は足りる？',
      a: [
        '初年度400件ペース（週8件）なら既存ルートセールスが生協訪問を兼務できる規模感。専任1名体制から始める想定。',
        '配送・設置は既存トラック・ドライバーが対応（廃棄物収集ルートと訪問先が重複）。',
        '整備（リファービッシュ）は埼玉倉庫スタッフが担当。追加採用は稼働300件超えてから検討で十分。',
      ],
    },
  ];
  ...
  qas.forEach((qa, i) => {
    const y = 0.82 + i * 1.58;
    card(s, 0.4, y, 9.2, 1.44, OFFWH, LGREY);
    card(s, 0.4, y, 0.07, 1.44, CORAL);
    // Q row
    slab(s, 'Q', 0.57, y + 0.08, 0.32, 0.30, CORAL);
    txt(s, qa.q, 1.00, y + 0.06, 8.42, 0.34,
      { fontSize: 13.5, bold: true, color: DTXT });
    hrule(s, 0.57, y + 0.46, 9.03 - 0.57);
    // A bullets
    qa.a.forEach((line, j) => {
      dot(s, 0.68, y + 0.60 + j * 0.28, CORAL);
      txt(s, line, 0.90, y + 0.52 + j * 0.28, 8.55, 0.28,
        { fontSize: 10, color: MTXT, lineSpacingMultiple: 1.15 });
    });
  });

  s.addNotes('よくある懸念と回答。①生協契約できる？→競合未提携校が多い・複数チャンネルあり・PoC先行。②留学生は借りる？→月2083円で参入障壁低い・生活必需品・市場実証済み。③誰が動く？→既存人員で初年度対応可能な規模感。');
}
...
// ══════════════════════════════════════════════════════════
{
  ...
  // 4 cards in 2×2 grid (compressed to make room for 4-month visit schedule)
  const CH = 1.55;  // card height
  const CG = 0.08;  // row gap
