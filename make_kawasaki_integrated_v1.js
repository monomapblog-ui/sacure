...
function kanjiCircle(s, kanji, x, y, col) {
  s.addShape(pres.ShapeType.ellipse, { x, y, w: 0.65, h: 0.65,
    fill:{ color: col }, line:{ type:'none' } });
  txt(s, kanji, x, y, 0.65, 0.65,
...
// ═══════════════════════════════════════════════════════════
{
  ...
  txt(s, '廃棄物データ × 都市センシング構想', 0.5, 0.88, 9.2, 1.0,
    { fontSize: 33, bold:true, color: WHITE });
  txt(s, '廃棄物量データ収集 × 配送トラック走行センサーデータ収集\n2つのデータで川崎市の都市管理・環境政策を支える', 0.5, 2.0, 9, 1.0,
    { fontSize: 15, color: 'B8CCE0', lineSpacingMultiple: 1.45 });
  ...
  txt(s, '廃棄物量データ　×　道路センシングデータ　×　川崎市との共創', 0, 5.08, 10, 0.545,
...
// ═══════════════════════════════════════════════════════════
{
  ...
  const pillars = [
    ...
    { kanji:'循', col: TEAL,
      title:'循環型経済の担い手',
      body:'廃棄物収集・リファービッシュ・再流通のサイクルを一貫運営。廃棄物量データの取得許可業者として唯一無二の立場にあります。' },
    { kanji:'信', col: STEEL,
      title:'行政・企業との協業実績',
  ...
  s.addNotes('会社紹介。200台以上の車両、250社超の法人取引実績。廃棄物収集許可業者として施設内データを合法的に取得できる唯一の立場。');
}
...
// ═══════════════════════════════════════════════════════════
{
  ...
  title(s, '2つのデータ収集事業を川崎市へご提案します', 0.5, 0.5, 9.2);
  hrule(s, 0.5, 1.22, 9.2);

  // Shared asset banner
  card(s, 0.5, 1.38, 9.2, 0.56, NAVY);
  accentBar(s, 0.5, 1.38, 0.56, TEAL);
  txt(s, '共通資産：200台以上の配送トラック × 250社超の廃棄物収集ネットワーク', 0.75, 1.38, 8.95, 0.56,
    { fontSize: 13, bold:true, color: WHITE, valign:'middle' });

  // Left card: 廃棄物量データ
  card(s, 0.5, 2.06, 4.45, 3.3, WHITE, LGREY);
  card(s, 0.5, 2.06, 4.45, 0.06, GREEN);
  kanjiCircle(s, '廃', 0.72, 2.18, GREEN);
  txt(s, '廃棄物量データ収集事業', 1.5, 2.2, 3.35, 0.46,
  ...
  wastePoints.forEach((pt, i) => {
    dot(s, 0.65, 2.76 + i * 0.46 + 0.04, GREEN);
    txt(s, pt, 0.9, 2.76 + i * 0.46, 3.9, 0.42,
      { fontSize: 11.5, color: DTXT });
  });

  // Right card: 走行センサーデータ
  card(s, 5.05, 2.06, 4.45, 3.3, WHITE, LGREY);
  card(s, 5.05, 2.06, 4.45, 0.06, TEAL);
  kanjiCircle(s, '路', 5.27, 2.18, TEAL);
  txt(s, '走行センサーデータ収集事業', 6.05, 2.2, 3.35, 0.46,
    { fontSize: 14, bold:true, color: TEAL2 });
  ...
  sensorPoints.forEach((pt, i) => {
    dot(s, 5.18, 2.76 + i * 0.46 + 0.04, TEAL);
    txt(s, pt, 5.45, 2.76 + i * 0.46, 3.9, 0.42,
      { fontSize: 11.5, color: DTXT });
  });

  // Center divider label
  txt(s, '×', 4.82, 3.3, 0.4, 0.4,
    { fontSize: 18, bold:true, color: MUTED, align:'center' });
...
// ═══════════════════════════════════════════════════════════
{
  ...
  title(s, '川崎市が直面する3つのデータ課題', 0.5, 0.5, 9.2);
  hrule(s, 0.5, 1.22, 9.2);

  const issues = [
    { n:1, col: NAVY,
      title:'パトロール・巡回体制の維持が困難',
      body:'自治体職員の減少と高齢化が進むなか、広大なエリアを高頻度でカバーする人的パトロールの継続は、人員・予算の両面から困難な状況にあります。' },
    { n:2, col: TEAL,
      title:'市民通報を主とする「事後対応」からの脱却',
      body:'道路陥没・落下物等のインフラ損傷は、市民通報を受けて初めて対応するケースが多く、損傷の早期発見・予防的な管理体制の構築が喫緊の課題です。' },
    { n:3, col: GREEN,
  ...
  s.addNotes('3つの課題：①パトロール体制の困難、②事後対応型管理、③環境施策・廃棄物管理のデータ不足。サキュレの2事業で解決を提案。');
}

// ═══════════════════════════════════════════════════════════
// SLIDE 5: 統合データフロー（2事業並列）
// ═══════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  card(s, 0,0,10,5.625, OFFWH);
  sectionLabel(s, '仕組み', 0.5, 0.22);
  title(s, '2つの事業が生み出すデータフロー', 0.5, 0.5, 9.2);
  hrule(s, 0.5, 1.22, 9.2);

  // Shared source header
  card(s, 0.5, 1.38, 9.2, 0.5, NAVY);
  accentBar(s, 0.5, 1.38, 0.5, TEAL);
  txt(s, '株式会社サキュレ　200台以上の配送トラック × 250社超の廃棄物収集ネットワーク', 0.75, 1.38, 8.95, 0.5,
    { fontSize: 12.5, bold:true, color: WHITE, valign:'middle' });

  // Left flow: 廃棄物量データ
  card(s, 0.5, 2.0, 4.45, 3.38, WHITE, LGREY);
  card(s, 0.5, 2.0, 4.45, 0.3, GREEN);
  txt(s, '廃棄物量データ', 0.5, 2.0, 4.45, 0.3,
  ...
  wasteSteps.forEach((st, i) => {
    const y = 2.42 + i * 0.9;
    numCircle(s, st.n, 0.65, y, GREEN);
    if (i < 2) {
      s.addShape(pres.ShapeType.line, { x: 0.87, y: y + 0.55, w: 0, h: 0.32,
        line:{ color: MUTED, width: 1.2 } });
    }
    txt(s, st.label, 1.3, y+0.04, 3.5, 0.36,
      { fontSize: 13, bold:true, color: DTXT });
    txt(s, st.sub, 1.3, y+0.42, 3.5, 0.28,
      { fontSize: 10.5, color: MTXT });
    if (i < 2) hrule(s, 0.65, y+0.82, 4.15);
  });

  // Right flow: 走行センサーデータ
  card(s, 5.05, 2.0, 4.45, 3.38, WHITE, LGREY);
  card(s, 5.05, 2.0, 4.45, 0.3, TEAL);
  txt(s, '走行センサーデータ', 5.05, 2.0, 4.45, 0.3,
    { fontSize: 12, bold:true, color: WHITE, align:'center', valign:'middle' });
  ...
  sensorSteps.forEach((st, i) => {
    const y = 2.42 + i * 0.72;
    numCircle(s, st.n, 5.18, y, TEAL);
    if (i < 3) {
      s.addShape(pres.ShapeType.line, { x: 5.4, y: y + 0.55, w: 0, h: 0.14,
        line:{ color: MUTED, width: 1.2 } });
    }
    txt(s, st.label, 5.82, y+0.04, 3.55, 0.34,
      { fontSize: 12.5, bold:true, color: DTXT });
    txt(s, st.sub, 5.82, y+0.39, 3.55, 0.26,
      { fontSize: 10.5, color: MTXT });
    if (i < 3) hrule(s, 5.18, y+0.68, 4.15);
  });

  s.addNotes('2事業のデータフロー。左：廃棄物量データ（3ステップ）、右：走行センサーデータ（4ステップ）。どちらも既存オペレーションに付随して収集。');
}
...
// ═══════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  card(s, 0,0,10,5.625, WHITE);
  sectionLabel(s, '走行センサーデータ', 0.5, 0.22);
  title(s, 'トラックが走るだけで3分野のデータが集まる', 0.5, 0.5, 9.2);
  hrule(s, 0.5, 1.22, 9.2);
  ...
  cats.forEach((cat, i) => {
    ...
    kanjiCircle(s, cat.kanji, 4.42, ry, cat.col);
...
// ═══════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  card(s, 0,0,10,5.625, OFFWH);
  sectionLabel(s, '廃棄物量データ', 0.5, 0.22);
  title(s, '廃棄物収集業務が生み出す都市環境データ', 0.5, 0.5, 9.2);
  hrule(s, 0.5, 1.22, 9.2);

  // Left explanation
  txt(s, '廃棄物量を\n都市の環境指標に変える', 0.5, 1.35, 4.2, 0.88,
    { fontSize: 19, bold:true, color: NAVY, lineSpacingMultiple: 1.25 });
  txt(s, '廃棄物収集許可業者として、\n施設内の廃棄物量・品目・頻度を\n合法的に記録・蓄積できます。\n自治体では入手できない施設別の\nリアルタイム廃棄物動向データを\n定期的に提供します。', 0.5, 2.3, 4.2, 1.55,
    { fontSize: 12, color: DTXT, lineSpacingMultiple: 1.4 });

  // Key benefit box
  card(s, 0.5, 3.96, 4.2, 1.42, GREEN);
  accentBar(s, 0.5, 3.96, 1.42, TEAL);
  txt(s, '追加のセンサー・人員・設備は不要\n廃棄物収集業務の「ついで」に都市環境データを生成', 0.75, 3.96, 3.95, 1.42,
    { fontSize: 12.5, bold:true, color: WHITE, valign:'middle', lineSpacingMultiple: 1.5 });

  // Right: 3 data value cards
  const dataVals = [
    { kanji:'量', col: GREEN,
      name:'廃棄物排出量トレンド',
  ...
  dataVals.forEach((d, i) => {
    const ry = 1.38 + i * 1.4;
    if (i > 0) hrule(s, 4.9, ry - 0.1, 4.8);
    kanjiCircle(s, d.kanji, 4.9, ry, d.col);
    txt(s, d.name, 5.68, ry + 0.02, 4.0, 0.42,
      { fontSize: 16, bold:true, color: d.col });
    txt(s, d.kw, 5.68, ry + 0.5, 4.0, 0.65,
      { fontSize: 11.5, color: MTXT, lineSpacingMultiple: 1.35 });
  });

  s.addNotes('廃棄物量データの3つの価値：量（排出量トレンド）・比（施設間比較）・域（地域別動向）。廃棄物収集許可業者として合法的・独占的に取得できるデータ。');
}
...
// ═══════════════════════════════════════════════════════════
{
  ...
  const strengths = [
    ...
    { n:3, col: GREEN, title:'現場知と独占的取得権',
      points:['廃棄物収集許可業者が取得可','現地の実態を熟知した人材','行政・企業との長期信頼関係'] },
  ];
  ...
  s.addNotes('3つの優位性：①網羅性と継続性、②追加投資不要の経済性、③現場知と独占的取得権（廃棄物収集許可業者）。');
}
...
// ═══════════════════════════════════════════════════════════
{
  ...
  txt(s, '走行センサーシステムは道路インフラ管理に特化した設計のもと、歩行者・車両の個人識別情報を一切保持しない運用体制を採ります。廃棄物量データは施設単位の集計値のみを扱います。', 0.75, 2.45, 3.25, 1.1,
    { fontSize: 11, color: 'C8DCE8', lineSpacingMultiple: 1.4 });
  ...
  txt(s, '準拠する主な法令', 0.75, 3.62, 3.2, 0.32,
    { fontSize: 10, color: TEAL, charSpacing: 0.5, bold:true });
  laws.forEach((law, i) => {
    dot(s, 0.75, 3.98 + i * 0.36 + 0.05, TEAL);
    txt(s, law, 1.0, 3.94 + i * 0.36, 3.05, 0.34,
  ...
  const items = [
    ...
    { n:3, col: GREEN, title:'行政との協議による運用体制の整備',
  ...
  s.addNotes('プライバシー・法令対応。3つの対策：①エッジAIによる自動マスキング、②暗号化・アクセス管理、③行政との協議による運用体制整備。廃棄物データは施設単位の集計値のみ。');
}
...
// ═══════════════════════════════════════════════════════════
{
  ...
  const pocs = [
    ...
    { label:'廃棄物収集施設', val:'10〜30施設\n川崎市内取引先から選定', col: GREEN },
  ];
  pocs.forEach((p, i) => {
    const x = 0.45 + i * 2.3;
    card(s, x, 1.38, 2.12, 1.95, WHITE, LGREY);
    card(s, x, 1.38, 2.12, 0.06, p.col);
    card(s, x+0.12, 1.52, 1.88, 0.28, LTBL);
    txt(s, p.label, x+0.12, 1.52, 1.88, 0.28,
      { fontSize: 9.5, color: TEAL2, align:'center', bold:true, valign:'middle', charSpacing:0.5 });
    txt(s, p.val, x+0.1, 1.85, 1.92, 0.88,
      { fontSize: 11, bold:true, color: NAVY, align:'center', lineSpacingMultiple: 1.3 });
  });
  ...
  const kpis = [
    { tag:'走行センサー', n:'KPI 1', text:'道路損傷の異常検知精度（検知率・誤検知率・平均検知所要時間）' },
    { tag:'走行センサー', n:'KPI 2', text:'道路管理工数の変化（巡回・点検・対応処理 前年度比）' },
    { tag:'廃棄物データ', n:'KPI 3', text:'廃棄物排出量データの精度・カバレッジ（施設数・品目網羅率）' },
  ];
  kpis.forEach((kpi, i) => {
    const y = 3.98 + i * 0.5;
    const tagCol = kpi.n === 'KPI 3' ? GREEN : TEAL;
    card(s, 0.5, y+0.04, 0.62, 0.32, tagCol);
    txt(s, kpi.n, 0.5, y+0.04, 0.62, 0.32,
      { fontSize: 9, bold:true, color: WHITE, align:'center', valign:'middle' });
    txt(s, kpi.text, 1.25, y+0.05, 8.35, 0.34,
      { fontSize: 12, color: DTXT });
    if (i < 2) hrule(s, 0.5, y+0.42, 9.2);
  });

  s.addNotes('PoC設計。4つのパラメータ：対象エリア・実施期間・センサー車両台数・廃棄物収集施設数。3つのKPI：走行センサー（検知精度・工数）、廃棄物データ（精度・カバレッジ）。');
}
...
// ═══════════════════════════════════════════════════════════
{
  ...
  title(s, '川崎市の都市管理に想定される変化（PoC検証予定）', 0.5, 0.5, 9.2);
  hrule(s, 0.5, 1.22, 9.2);

  const outcomes = [
    { n:1, col: NAVY, title:'道路インフラの予防保全',
      bef:'市民通報が主な情報源',
      aft:'異常の予兆を早期に自動検知' },
    { n:2, col: TEAL, title:'巡回・監視コストの削減',
      bef:'人的パトロール中心の運用',
      aft:'自動収集が補完し\n職員は対応業務に集中' },
    { n:3, col: GREEN, title:'廃棄物・環境KPIの高度化',
      bef:'廃棄物量は把握しにくく\n施策効果を定量化困難',
      aft:'施設別データで\n環境KPIを定量的に検証' },
  ];
...
// ─── Output ───────────────────────────────────────────────
const OUT = '/home/user/sacure/川崎市_統合提案資料_v1.pptx';
